...
function Header() {
    ...
    return (
        <div className='header'>
            <div><b>OnlineShop</b></div>
            <div>
                {cartCounter > 0 ? <span className='cart-counter'>{cartCounter}</span> : null}
                <FontAwesomeIcon icon={faShoppingCart} color='White' size='2x' onClick={cartShow} />
            </div>
        </div>
    );
}
